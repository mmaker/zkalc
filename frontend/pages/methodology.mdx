...
For numbers outside the benchmarking range, we [extrapolate](https://en.wikipedia.org/wiki/Extrapolation) via non-linear least squares. To make things more exciting we decided that it should be done... in [Javascript](https://github.com/asn-d6/zkalc/blob/main/frontend/lib/estimates.js) inside your browser.
...
In the specific case of MSMs, Pippenger's complexity is [well known](https://jbootle.github.io/Misc/pippenger.pdf) to be asymptotically <InlineMath math="O({n} / {\log n})" />. Hence, we fit the data set to a function <InlineMath math="h(x) = \frac{a x + b}{\log x}" />
