...
1. For each supported operation, we run benchmarks that measures its performance.
    We use [`criterion.rs`](https://criterion.rs), to take at least 10 samples, and then select the median.
1. We collect the data inside `frontend/data/` and make it freely available.
1. We extrapolate a curve linking the aboves samples, using linear interpolation inside the curve and curve fitting with least squares outside the benchmarking bounds.
1. When a user queries zkalc for an operation of size <InlineMath math="n" />, we estimate the its running time using the produced function <InlineMath math="n" />.
...
That is, for each pair of benchmark data <InlineMath math="(x_i, f(x_i))" /> and <InlineMath math="(x_{i+1}, f(x_{i+1}))" /> we trace the line [that goes through both points](https://github.com/asn-d6/zkalc/blob/main/perf/fit.py).
We end up with a piecewise function that covers the entire benchmark range, as we can see in the figure below:

<PlotPointsAndEstimates />

For numbers outside the benchmarking range, we [extrapolate](https://en.wikipedia.org/wiki/Extrapolation) via non-linear curve fitting with least squares, which to our surprise, given the simple asymptotic can also be done in [javascript](https://github.com/asn-d6/zkalc/blob/main/frontend/lib/estimates.js). Here is an example of the behavior of the MSM estimations outside of the benchmarking range.

<PlotExtrapolation />

In the specific case of MSMs, Pippenger's complexity is [well known](https://jbootle.github.io/Misc/pippenger.pdf) to behave like <InlineMath math="\frac{n}{\log n}" />. we fit the data set to a function <InlineMath math="h(x) = \frac{ax + b}{logx}" /> and we evaluate it at <InlineMath math="h(n)" />.

We do not expect extrapolation to faithfully follow the benchmarks. For instance, in the case of multi-scalar multiplications as above, we record an error of XXX for the size YYY. We believe however that the estimates provide a rough idea of how long an algorithm will take.
In addition, it's worth pointing out while we try to provide useful results for arbitrary operation sizes, we cannot guarantee that the requested machine will be able to handle the requested size. For example, a commodity laptop will likely run out of memory before it computes an MSM of size <InlineMath math="2^{28}" />, unless the user resorts to streaming algorithms.

## Hazard zone

If you open the code
...
export default ({ children }) => <Layout title="zkalc's methodology">{children}</Layout>
